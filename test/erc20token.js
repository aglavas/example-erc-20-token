...
contract('ERC20Token', accounts => {
  ...
  it('should NOT transfer token if balance too low', async () => {
    await expectRevert(token.transfer(accounts[1], web3.utils.toWei('10')), 'Token balance too low.');
  });

  it('should NOT transfer token if not approved', async () => {
    await expectRevert(token.transferFrom(accounts[0], accounts[1], 10), 'Token balance too low.');
  });
  ...
  it('should transfer token when approved', async () => {
    ...
    receipt = await token.approve(accounts[1], _100, {from: accounts[0]});
    allowance = await token.allowance(accounts[0], accounts[1]);
    assert(allowance.eq(_100));
    expectEvent(receipt, 'Approval', {tokenOwner: accounts[0], spender: accounts[1], tokens: _100});

    receipt = await token.transferFrom(accounts[0], accounts[2], _100, {from: accounts[1]});
    allowance = await token.allowance(accounts[0], accounts[1]);
    ...
    assert(allowance.isZero());
    expectEvent(receipt, 'Transfer', {from: accounts[0], to: accounts[2], tokens: _100});
  });
});